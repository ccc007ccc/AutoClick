...
def click_option():
    ...
    try:
        x, y = pag.locateCenterOnScreen(
            option, region=(616, 325, 1250, 810), grayscale=True, confidence=0.9
        )
...
def click_next(int):  # 传入循环次数
    ...
    # 如果没有找到，抛出异常
        ...
        print(
            "Failed to read "
            + str(int)
            + ".png because file is missing, has improper permissions, or is an unsupported or invalid format"
        )
        return False
